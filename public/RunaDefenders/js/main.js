...
function gameLoop() { if (gameState === 'playing') { levelTimer += 1/60; globalAnimationTimer += 0.02; if (shootTimer > 0) shootTimer--; handleCollisions(); handleLevelProgression(); updateUI(base, currentLevelIndex, coffeeBeanCount, specialPowerPoints, isPowerActive, levelTimer, config); } draw(); if (base.health <= 0) { gameState = 'game_over'; stopMusic(); showOverlay('game_over'); } else { animationFrameId = requestAnimationFrame(gameLoop); } }
