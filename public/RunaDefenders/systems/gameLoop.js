...
// La función que actualiza la interfaz de usuario
function updateUI(gameContext) {
    // Esta lógica la moveremos al archivo ui.js más adelante
    // Por ahora, la dejamos aquí para que funcione
    const { levelValue, coffeeBeanCounterEl, specialPowerBar } = gameContext.uiElements;
    levelValue.textContent = gameContext.currentLevelIndex + 1;
    coffeeBeanCounterEl.textContent = gameContext.coffeeBeanCount;
    specialPowerBar.style.width = `${(gameContext.specialPowerPoints / gameContext.config.specialPowerMax) * 100}%`;
    // ... más lógica de UI
}
